    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-5 py-8 space-y-5">
        <div className="card text-center space-y-5">
          ...
          <div
            className={`font-black text-sm px-5 py-3 rounded-2xl ${
              skor >= 80
                ? 'bg-duo-green-light text-duo-green-dark'
                : skor >= 50
                  ? 'bg-duo-orange-light text-duo-orange'
                  : 'bg-duo-red-light text-duo-red'
            }`}
          >
            {skor >= 80
              ? 'Cemerlang! Anda faham subtopik ini.'
              : skor >= 50
                ? 'Usaha yang baik! Ada ruang untuk penambahbaikan.'
                : 'Jangan putus asa. Cuba semula selepas baca nota.'}
          </div>
        </div>

        {Object.keys(subStats).length > 0 && (
          <div className="card space-y-4">
            <h2 className="text-sm font-black text-duo-charcoal dark:text-gray-100">
              Prestasi Mengikut Sub-Kemahiran
            </h2>
                  ...
                  <div key={sk} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {isLemah && (
                          <span className="badge bg-duo-red-light dark:bg-duo-red/20 text-duo-red">
                            Lemah
                          </span>
                        )}
                      </div>
                      <span
                        className={`text-sm font-black ${warna === 'green' ? 'text-duo-green-dark dark:text-duo-green' : warna === 'yellow' ? 'text-duo-orange' : 'text-duo-red'}`}
                      >
                        {betul}/{jml}
                      </span>
                    </div>
        ...
        <div
          className={`rounded-2xl border-2 p-5 space-y-3 ${
            cadangan.jenis === 'teruskan'
              ? 'border-duo-green dark:border-duo-green bg-duo-green-light dark:bg-duo-green/10'
              : cadangan.jenis === 'latihan_fokus'
                ? 'border-duo-orange dark:border-duo-orange/30 bg-duo-orange-light dark:bg-duo-orange/10'
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-black text-duo-charcoal dark:text-gray-100 text-sm">
                Cadangan Seterusnya
              </h3>
              <p className="text-sm font-semibold text-duo-charcoal/70 dark:text-gray-300 mt-0.5">
                {cadangan.mesej}
              </p>
            </div>
          </div>
          ...
          {cadangan.kemahiran_lemah.length > 0 && (
            <div className="pl-13">
              <p className="text-xs font-bold text-duo-charcoal/50 dark:text-gray-400 mb-1">
                Sub-kemahiran perlu diperbaiki:
              </p>
              <div className="flex flex-wrap gap-1.5">
                {cadangan.kemahiran_lemah.map((k) => (
                  <span
                    key={k}
                    className="badge bg-duo-red-light dark:bg-duo-red/20 text-duo-red capitalize"
                  >
                    {k.replace(/_/g, ' ')}
                  </span>
                ))}
          ...
          <div className="space-y-2">
            {subtopik.soalan.map((s, i) => {
              ...
              return (
                <div
                  key={s.id}
                  className={`flex items-start gap-3 p-3 rounded-2xl ${isBetul ? 'bg-duo-green-light' : 'bg-duo-red-light'}`}
                >
                  <div
                    className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-0.5 ${isBetul ? 'bg-duo-green' : 'bg-duo-red'}`}
                  >
                    {isBetul ? (
                      <svg
                        className="w-5 h-5 text-white"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-duo-charcoal/80 dark:text-gray-200">
                      S{i + 1}: {s.soalan}
                    </p>
                    {!isBetul && (
      ...
      <div className="sticky bottom-0 z-10 bg-white dark:bg-duo-charcoal border-t-2 border-duo-gray-light dark:border-white/10 px-5 py-4">
        <div className="max-w-lg mx-auto flex gap-3">
          <button onClick={() => navigate(`/kuiz/${subtopikId}`)} className="btn-secondary flex-1">
            Cuba Semula
          </button>
