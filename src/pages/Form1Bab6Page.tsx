    ...
    >
      <div
        className={`w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-black border-2 transition-colors ${
            ...
            ? 'bg-duo-purple border-duo-purple text-white'
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-sm text-duo-charcoal dark:text-gray-100 leading-snug group-hover:text-duo-purple-dark dark:group-hover:text-duo-purple transition-colors truncate">
          {lesson.tajuk}
        </p>
...
export function Form1Bab6Page() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [idx, progs] = await Promise.all([fetchChapterIndex(1, 6), getAllLessonProgress()])
        setIndex(idx)
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <AppHeader tajuk="Tingkatan 1 — Bab 6" showBack backTo="/" />
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-5 py-6 space-y-6">
        <div className="rounded-2xl bg-gradient-to-br from-duo-purple to-duo-purple-dark text-white p-6 shadow-lg">
          ...
          <h2 className="text-2xl font-black mt-1">Persamaan Linear</h2>
          <p className="text-white/80 text-sm mt-1">
            Persamaan Linear Satu Pemboleh Ubah, Dua Pemboleh Ubah, dan Persamaan Serentak
          </p>
          {totalCount > 0 && (
