...
function LessonCard({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left flex items-center gap-4 px-4 py-3.5 rounded-2xl border-2 bg-white dark:bg-white/5 transition-all duration-150 hover:border-duo-blue hover:shadow-md active:scale-[0.98] group cursor-pointer border-duo-gray-light dark:border-white/10"
    >
      {/* Status circle */}
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-sm text-duo-charcoal dark:text-gray-100 leading-snug group-hover:text-duo-blue-dark dark:group-hover:text-duo-blue transition-colors truncate">
          {lesson.tajuk}
        </p>
      ...
      {score !== null ? (
        ...
        <svg
          className="w-4 h-4 text-duo-gray flex-shrink-0 group-hover:text-duo-blue transition-colors"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
...
export function Form1ChapterPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [idx, progs] = await Promise.all([fetchChapterIndex(1, 1), getAllLessonProgress()])
        setIndex(idx)
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <AppHeader tajuk="Tingkatan 1 — Bab 1" showBack backTo="/" />

      <main className="flex-1 max-w-lg mx-auto w-full px-5 py-6 space-y-6">
        {/* Chapter hero */}
        <div className="rounded-2xl bg-gradient-to-br from-duo-green-dark to-duo-green text-white p-6 shadow-lg">
          <p className="text-white/80 text-xs font-bold uppercase tracking-wide">
            Matematik Tingkatan 1 · Bab 1
          </p>
          <h2 className="text-2xl font-black mt-1">Nombor Nisbah</h2>
          <p className="text-white/80 text-sm mt-1">
            Integer, Pecahan, Perpuluhan dan Nombor Nisbah
          </p>
          {totalCount > 0 && (
